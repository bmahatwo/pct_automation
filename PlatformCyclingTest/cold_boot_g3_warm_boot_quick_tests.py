...
if os.path.isdir(originalFolderWithCompletePath):
	...
	stringFolderName_NowTimeDate = str(currentIteration) + "_" + nowTimeDate.strftime("%d%m%Y_%H%M%S") + "_" + constants.STRING_COLD_BOOT_FROM_G3_WARM_BOOT_QUICK
	...
	# Odd number currentIteration (1, 3, 5, 7...): Cold boot: prev_sleep_state 5
	# Even number currentIteration (0, 2, 4, 6...): Warm boot: prev_sleep_state 0
	if (currentIteration % 2 == 0):
		logger.info (iterationFolderCompletePath + ": " + fileName + ": " + functionName + ": line " + str(frameObj.f_lineno) + ": Received even number currentIteration")
		execute_save_compare.compare_prev_sleep_state(constants.PREV_SLEEP_STATE_COLD_BOOT, iterationFolderCompletePath)
	else:
		logger.info (iterationFolderCompletePath + ": " + fileName + ": " + functionName + ": line " + str(frameObj.f_lineno) + ": Received odd number currentIteration")
		execute_save_compare.compare_prev_sleep_state(constants.PREV_SLEEP_STATE_WARM_BOOT, iterationFolderCompletePath)
...
# Odd number currentIteration (1, 3, 5, 7...): Warm boot. So, initiate next one for cold boot
# Even number currentIteration (0, 2, 4, 6...): Cold boot. So, initiate next one for warm boot
if (currentIteration%2 == 1):
	...
	logger.info (fileName + ": " + functionName + ": line " + str(frameObj.f_lineno) + ": after calling reboot")

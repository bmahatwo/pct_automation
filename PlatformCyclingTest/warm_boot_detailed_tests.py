...
if os.path.isdir(originalFolderWithCompletePath):
	...
	stringFolderName_NowTimeDate = str(currentIteration) + "_" + nowTimeDate.strftime("%d%m%Y_%H%M%S") + "_" + constants.STRING_WARM_BOOT_DETAILED
	...
	execute_save_compare.measure_time_between_each_iteration(constants.CONST_ITERATION_TIME_OF_WARM_BOOT_DETAILED, iterationFolderCompletePath)
	...
	# Cold boot: prev_sleep_state 5
	# Warm boot: prev_sleep_state 0
	execute_save_compare.compare_prev_sleep_state(constants.PREV_SLEEP_STATE_WARM_BOOT, originalFolderWithCompletePath)
